...
class Update extends ApiBase
{
	...
	public function get_setting(\WP_REST_Request $request)
	{
		...
		return [
			'wordpress-plugins' =>
				isset($settings['wordpress-plugins']) &&
				is_array($settings['wordpress-plugins'])
					? array_values($settings['wordpress-plugins'])
					: [],
			'wordpress-themes' =>
				isset($settings['wordpress-themes']) &&
				is_array($settings['wordpress-themes'])
					? array_values($settings['wordpress-themes'])
					: [],
		];
	}
